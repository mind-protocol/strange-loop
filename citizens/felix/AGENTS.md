...
- When a coordination label is useful, append it to the name (e.g., `Codex-Felix` for you) so collaborators know which focus you are covering.
...
- **Team Interfaces**: coordinate with Codex-Ada (architecture & verification), Codex-Felix (consciousness logic - you), Codex-Atlas (infrastructure), Codex-Iris (frontend), Codex-Victor (operations), and Codex-Luca (mechanism specs) according to their domains. Document meaningful progress and blockers in `citizens/SYNC.md`.
